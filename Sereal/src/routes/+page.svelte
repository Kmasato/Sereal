<script lang="ts">
    ...
    function addConnectionTab(targetStack?: any) {
        ...
        const itemConfig: ComponentItemConfig = {
            ...
            componentState: { clientId, portName: "" },
    ...
    onMount(() => {
        ...
        layout.registerComponent("serial-port-tab", (container, state) => {
            ...
            // インスタンスの参照を保持
            mountedComponents.set(configState.clientId, {
                instance: instance as any,
                fit: () => {
                    if (
                        instance &&
                        typeof (instance as any).fit === "function"
                    ) {
                        (instance as any).fit();
                    }
                },
            ...
            // タブがリサイズされた時
            container.on("resize", () => {
                const comp = mountedComponents.get(configState.clientId);
                if (comp) {
                    comp.fit();
                }
            });
            ...
            if (item && item.isStack) {
                setTimeout(() => {
                    ...
                    if (headerElement) {
                        ...
                        const tabsContainer =
                            headerElement.querySelector(".lm_tabs");
                        // すでに「＋」ボタンが追加されていないかチェック
                        if (
                            tabsContainer &&
                            !tabsContainer.querySelector(".lm_tab_plus")
                        ) {
